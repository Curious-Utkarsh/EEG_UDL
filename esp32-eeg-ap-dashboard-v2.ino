...
#define QUEUE_DEPTH         (SAMPLE_RATE_HZ * 4)
...
typedef struct { uint16_t value; uint32_t timestamp; } Sample_t;
...
function handleStatus(payload) {
  ...
  if (type === 'LOG_STARTED') {
    ...
    logging     = true;
    csvBatches  = ['SampleNo,Timestamp_ms,ADC_Raw,Voltage_V\n'];  // CSV header first
...
void graphBatchAdd(const Sample_t& s) {
  char tmp[32];
  int len = snprintf(tmp, sizeof(tmp), "%s{\"v\":%u,\"t\":%lu}",
                     graphBufCount > 0 ? "," : "",
                     s.value, (unsigned long)s.timestamp);
...
void adcSampleTask(void* parameter) {
  ...
  for (;;) {
    ...
    s.timestamp = millis();
    // Never block — drop sample if queue is full rather than stall the task
...
void setup() {
  ...
  // ADC: 12-bit, 0–3.3 V (11 dB attenuation)
  analogReadResolution(12);
  analogSetPinAttenuation(ADC_PIN, ADC_11db);
  pinMode(ADC_PIN, INPUT);

  // ── ADC self-test: gives you instant wiring feedback on Serial Monitor ─
  Serial.printf("[ADC] Self-test on GPIO%d (50 reads)...\n", ADC_PIN);
  ...
  Serial.printf("[ADC] min=%u  max=%u  avg=%u  (%.3f V avg)\n",
                lo, hi, sum/50, (sum/50)*3.3f/4095.0f);
  if (hi < 10)         Serial.println("[ADC] WARNING: all reads near 0 — check wiring/GND");
  else if (lo == hi)   Serial.println("[ADC] WARNING: values static — pin may be floating");
  else                 Serial.println("[ADC] OK");
  ...
  Serial.printf("[Setup] Sampling GPIO%d at %d Hz. Dashboard ready.\n",
